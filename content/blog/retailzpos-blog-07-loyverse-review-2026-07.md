...
If you're a single-location store with a simple product mix, minimal age-restricted complexity, and loyalty needs that amount to "track repeat customers and give them something back occasionally," Loyverse's free tier can carry you a long way. There's no shame in starting here, and plenty of small operators run on it successfully.

## Where it runs out

**Inventory for case-break categories.** Loyverse's inventory tools are built for general retail. Liquor retail specifically needs clean case-to-bottle conversion tracking — Loyverse doesn't have this built in the way a liquor-specific system does.

**Loyalty mechanics stay basic.** The built-in barcode/card loyalty system is a step behind more modern no-card, phone-number-based enrollment models, and there's no category-specific reward logic — no equivalent to a wine-tasting bonus or a vape-bundle trigger. It tracks visits; it doesn't shape behavior.

**No purpose-built age verification.** Loyverse isn't designed around age-restricted retail. If you're selling liquor, tobacco, or vape products, the register isn't enforcing age checks the way a hard-block system built for regulated retail does — see our [POS compliance guide] for why that distinction is worth taking seriously, not just a nice-to-have.
...
**Is Loyverse actually free, or is there a catch?**
The core POS and basic barcode/card loyalty program are genuinely free. Paid add-ons exist for additional functionality, but there's no hidden baseline subscription requirement to start using it.

**Can Loyverse handle a liquor store's inventory needs?**
It can track basic inventory, but case-to-bottle conversion — a liquor-specific need — isn't built in the way it is with a liquor-specific POS. Test it against your actual SKU list before relying on it for this.
